...
hayMina s (a,b) = encuentraX b (encuentraY a s)

-- Auxiliar de hayMina
encuentraX :: Integer -> [Bool] -> Bool
encuentraX n k | n == 1 = head k
               | otherwise = encuentraX (n-1) (tail k)
-- Auxiliar de hayMina
encuentraY :: Integer -> [[Bool]] -> [Bool]
encuentraY n k | n == 1 = head k
               | otherwise = encuentraY (n-1) (tail k)
...
--Auxiliar que busca el tablero por columna
auxlistaX :: Integer -> [Desplazamiento] -> Desplazamiento
auxlistaX n k | n == 1 = head k
              | otherwise = auxlistaX (n-1) (tail k)
--Auxiliar que busca el tablero por fila
auxlistaY :: Integer -> [[Desplazamiento]] -> [Desplazamiento]
auxlistaY n k | n == 1 = head k	
              | otherwise = auxlistaY (n-1) (tail k)
...
recorrido n (a,b)|posValida n (a,b) == False = [] 
                 |otherwise = (a,b) :recorrido n (moverse (a,b) (auxlistaX b (auxlistaY a n)))

--Auxiliar de escapaDelTablero para que si despues de una cantidad de movimientos no se escapo es falso.
escapaAux :: TableroAF -> Posicion -> Integer -> Bool
escapaAux k (a,b) n| n == 0 = False
		   | posValida k (a,b) == False = True
		   |otherwise = escapaAux k (moverse (a,b) (auxlistaX b (auxlistaY a k))) (n-1)
...
--cambio de direccion de una flecha
...
--encuentra columna del tablero
remplazarA :: TableroAF -> [Desplazamiento] -> Integer -> TableroAF
remplazarA k s n| n == 1 = s : (tail k)
		|otherwise =  (head k) : remplazarA (tail k) s (n-1)

--encuentra Fila del tablero
remplazarB :: [Desplazamiento] -> Desplazamiento -> Integer -> [Desplazamiento]
remplazarB s r n|n == 1 = r : (tail s)
		|otherwise = (head s) : remplazarB (tail s) r (n-1)
...
cambiaFlechaTablero k (a,b) = remplazarA k (remplazarB k (cambiaFlecha (auxlistaX b (auxlistaY a k)))) a
...
cantidadDePasosParaSalirAux k (a,b) n|posValida k (a,b) == False = n
				     |otherwise = cantidadDePasosParaSalirAux (cambiaFlechaTablero k (a,b)) (moverse (a,b) (auxlistaX b (auxlistaY a k))) (n+1)
